...
from facefusion.face_helper import warp_face_by_face_landmark_5, warp_face_by_translation, create_static_anchors, \
	distance_to_face_landmark_5, distance_to_bounding_box, convert_face_landmark_68_to_5, apply_nms, categorize_age, \
	categorize_gender
	...
	with THREAD_LOCK:
		...
		if FACE_ANALYSER is None:
			...
			if facefusion.globals.face_detector_model in ['many', 'scrfd']:
				face_detector = onnxruntime.InferenceSession(MODELS.get('face_detector_scrfd').get('path'),
															 providers=apply_execution_provider_options(
																 facefusion.globals.execution_providers))
				face_detectors['scrfd'] = face_detector
			if facefusion.globals.face_detector_model in ['many', 'yoloface']:
				face_detector = onnxruntime.InferenceSession(MODELS.get('face_detector_yoloface').get('path'),
															 providers=apply_execution_provider_options(
																 facefusion.globals.execution_providers))
			...
			if facefusion.globals.face_recognizer_model == 'arcface_uniface':
				face_recognizer = onnxruntime.InferenceSession(
					...
					providers=apply_execution_provider_options(facefusion.globals.execution_providers))
			face_landmarker = onnxruntime.InferenceSession(MODELS.get('face_landmarker').get('path'),
														   providers=apply_execution_provider_options(
															   facefusion.globals.execution_providers))
...
def detect_with_retinaface(vision_frame: VisionFrame, face_detector_size: str) -> Tuple[
...
def detect_with_scrfd(vision_frame: VisionFrame, face_detector_size: str) -> Tuple[
	List[BoundingBox], List[FaceLandmark5], List[Score]]:
...
def detect_with_yoloface(vision_frame: VisionFrame, face_detector_size: str) -> Tuple[
	List[BoundingBox], List[FaceLandmark5], List[Score]]:
...
def detect_with_yunet(vision_frame: VisionFrame, face_detector_size: str) -> Tuple[
	List[BoundingBox], List[FaceLandmark5], List[Score]]:
...
def create_faces(vision_frame: VisionFrame, bounding_box_list: List[BoundingBox],
				 face_landmark_5_list: List[FaceLandmark5], score_list: List[Score]) -> List[Face]:
	faces = []
...
def calc_embedding(temp_vision_frame: VisionFrame, face_landmark_5: FaceLandmark5) -> Tuple[Embedding, Embedding]:
	...
	crop_vision_frame, matrix = warp_face_by_face_landmark_5(temp_vision_frame, face_landmark_5, 'arcface_112_v2',
															 (112, 112))
...
def get_one_face(vision_frame: VisionFrame, position: int = 0, **kwargs) -> Optional[Face]:
	many_faces = get_many_faces(vision_frame, **kwargs)
	if many_faces:
		try:
...
def get_average_face(vision_frames: List[VisionFrame], position: int = 0, **kwargs) -> Optional[Face]:
	average_face = None
	faces = []
	embedding_list = []
	...
	for vision_frame in vision_frames:
		face = get_one_face(vision_frame, position, **kwargs)
		if face:
			faces.append(face)
...
def get_many_faces(vision_frame: VisionFrame, **kwargs) -> List[Face]:
	faces = []
	try:
		...
		if faces_cache:
			...
			face_landmark_5_list = []
			score_list = []
			face_detector_model = kwargs.get('face_detector_model', facefusion.globals.face_detector_model)
			if face_detector_model in ['many', 'retinaface']:
				bounding_box_list_retinaface, face_landmark_5_list_retinaface, score_list_retinaface = detect_with_retinaface(
					vision_frame, kwargs.get('face_detector_size', facefusion.globals.face_detector_size))
			...
			if face_detector_model in ['many', 'scrfd']:
				bounding_box_list_scrfd, face_landmark_5_list_scrfd, score_list_scrfd = detect_with_scrfd(
					vision_frame,
					kwargs.get('face_detector_size', facefusion.globals.face_detector_size)
				)
			...
			if face_detector_model in ['many', 'yoloface']:
				bounding_box_list_yoloface, face_landmark_5_list_yoloface, score_list_yoloface = detect_with_yoloface(
					vision_frame, kwargs.get('face_detector_size', facefusion.globals.face_detector_size))
			...
			if face_detector_model in ['yunet']:
				bounding_box_list_yunet, face_landmark_5_list_yunet, score_list_yunet = detect_with_yunet(
					vision_frame,
					kwargs.get('face_detector_size', facefusion.globals.face_detector_size)
				)
		...
		if kwargs.get('face_analyser_order', facefusion.globals.face_analyser_order):
			faces = sort_by_order(faces, kwargs.get('face_analyser_order', facefusion.globals.face_analyser_order))
		if kwargs.get('face_analyser_age', facefusion.globals.face_analyser_age):
			faces = filter_by_age(faces, kwargs.get('face_analyser_age', facefusion.globals.face_analyser_age))
		if kwargs.get('face_analyser_gender', facefusion.globals.face_analyser_gender):
			faces = filter_by_gender(faces, kwargs.get('face_analyser_gender', facefusion.globals.face_analyser_gender))
	except (AttributeError, ValueError):
		pass
	return faces


def find_similar_faces(reference_faces: FaceSet, vision_frame: VisionFrame, face_distance: float, **kwargs) -> List[Face]:
